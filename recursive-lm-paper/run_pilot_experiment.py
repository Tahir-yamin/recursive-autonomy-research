...
# Structured logger for diagnostics. Error/warning sites route through this
# logger; remaining print() calls are intentional CLI progress output.
logging.basicConfig(
    level=logging.INFO,
            ...
            try:
                async with actual_session.post(url, headers=headers, json=payload, timeout=30) as response:
                    if response.status == 200:
                        ...
                        log.error("API Error %s: %s", response.status, await response.text())
...
def parse_json_response(response_text):
    ...
    try:
        ...
        log.warning("Brace-matching JSON parsing failed: %s", e)
    return None
...
def save_failed_payload(prompt, error_msg="Timeout or API Error"):
    ...
    try:
        ...
        log.error("Failed to log failed payload: %s", e)
...
def save_wal(condition, seed, cycle, trials, densities, net_tokens=0, custom_state=None, campaign_results=None):
    ...
    for attempt in range(5):
        try:
            ...
            last_err = e
            if attempt < 4:
                # exponential backoff WITH jitter so concurrent writers to the
                # same target do not re-collide in lockstep on Windows locks.
    ...
    if replaced:
        ...
        # Atomic replace failed on every attempt. Do NOT delete tmp_file: it holds
        # the only copy of the new checkpoint. Surface the failure (raise) instead
        # of silently returning, so the loss is never silent. The per-cycle caller
        # logs-and-continues and tmp_file remains as a recovery copy.
        log.error("SRE WAL Error: failed to replace %s after 5 attempts; "
                  "preserving recovery copy %s.", wal_file, tmp_file)
    ...
    if os.path.exists(wal_file):
        try:
            ...
            if computed != checksum:
                log.error("WAL integrity verification failed! Checksum mismatch.")
                return None
            return state
        except (OSError, ValueError) as e:
            log.error("Failed to read WAL: %s", e)
    return None
...
def clear_wal(wal_file):
    ...
    if os.path.exists(wal_file):
        try:
            ...
            log.warning("Failed to clear WAL: %s", e)
...
"""
            ...
            log.error("SRE Background Worker Crash: %s", e)
...
async def execute_campaign():
    ...
    for cond in campaign_results["conditions"]:
        campaign_results["conditions"][cond] = {
            ...
            "net_tokens": [],
            "generalization_gaps": [],
            # Per-cycle trajectories (one list-of-dicts per seed) so context-rot
            # dynamics -- density growth, cumulative redundancy, running-best
            # accuracy -- are measurable post hoc rather than collapsed to a
            # per-seed mean. Captured for non-resumed runs.
...
"""
                        ...
                        # Persist the per-cycle state needed to measure context rot:
                        # prompt density, redundancy, and running-best accuracy.
                        cycle_trace.append({
                            "cycle": cycle,
                    ...
                    if cond == "rar_compressed" and consolidator.task:
                        try:
                            ...
                            log.error("Error awaiting final consolidator task: %s", e)
                    ...
                    if consolidator.task and not consolidator.task.done():
                        ...
                        try:
                            ...
                            log.error("SRE Clean: Error during worker cancellation: %s", e)
    ...
    if len(SEEDS) >= 2:
        try:
            ...
            # wilcoxon raises ValueError when all paired differences are zero
            # (no variance). Narrow the catch and log it so a genuine test
            # failure is never silently coerced into a significance-looking string.
            log.warning("Wilcoxon test not computable (%s); reporting n.s.", e)

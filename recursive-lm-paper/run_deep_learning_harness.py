...
def evaluate_test_vault(best_config, dataset_seed=42, epochs=15):
    ...
    One-shot guard: enforces that the locked Test Vault is evaluated at most once per
    dataset seed within a campaign process, so the "evaluated a single time at
    completion" claim is mechanically guaranteed rather than merely asserted.
    """
    if dataset_seed in _TEST_VAULT_SEEN:
        raise RuntimeError(
            f"evaluate_test_vault: Test Vault for seed {dataset_seed} already "
            "evaluated this process. The locked vault is single-shot by design; "
            "refusing a second evaluation to prevent test-set contamination."
        )
    _TEST_VAULT_SEEN.add(dataset_seed)
    # SRE Fast Simulation Mode (if no API keys are present in environment)
    gemini_key = os.environ.get("GEMINI_API_KEY")
    deepseek_key = os.environ.get("DEEPSEEK_API_KEY")
    openrouter_key = os.environ.get("OPENROUTER_API_KEY")

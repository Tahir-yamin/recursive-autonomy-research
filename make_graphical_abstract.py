"""One-page graphical abstract for sharing (vector PDF), aligned to the current
three-benchmark framing. Numbers trace to pilot_results_taskA.json (the real
Task A headline) and the abstract in main.tex.
"""
import json, os
import numpy as np
import matplotlib
...
def conds(fn):
    ...
    return (d.get("data", d).get("conditions") or d.get("conditions"))

C = conds("pilot_results_taskA.json")
...
fig.text(0.5, 0.915, "Keep a long-running AI agent affordable by compressing its own history "
         "as it works — same accuracy, roughly half the cost.",
         ha="center", fontsize=9.5, color="#48606b")

# ---- left: problem -> fix schematic -----------------------------------------
...
axL.text(5, 1.7, "Tested on three benchmarks of graded difficulty (tuned synthetic\n"
         "manifold, CIFAR-10→64 PCs, sklearn digits), 10 seeds × 60 cycles each.\n"
         "RAR summarises its log and stores past results in a small graph.",
         ha="center", va="center", fontsize=7.4, color="#48606b")
...
axT.annotate(f"−{red:.0f}% cost", xy=(1, rar_tok/1e3), xytext=(0.42, base_tok/1e3*0.82),
             fontsize=9, color=TEAL, fontweight="bold",
             arrowprops=dict(arrowstyle="->", color=TEAL, lw=1.2))
for s in ["top", "right"]: axT.spines[s].set_visible(False)
axT.set_title(f"Validated: ~{red:.0f}% cheaper (up to 68% on the easiest task)",
              fontsize=9, color=INK, loc="left")

# ---- right bottom: accuracy parity ------------------------------------------
axB = fig.add_axes([0.55, 0.12, 0.40, 0.26])
...
axB.text(base_acc, 0.93, "±2 pp parity band", ha="center", fontsize=7, color="#48606b")
axB.text(base_acc, 0.6, "  baseline", va="center", fontsize=8, color=GRAY)
axB.text(rar_acc, 0.4, "  RAR", va="center", fontsize=8, color=TEAL)
axB.set_ylim(0.2, 1.05); axB.set_yticks([]); axB.set_xlim(base_acc-4, base_acc+4)
axB.set_xlabel("Mean test accuracy (%) — Task A", fontsize=8.5)
for s in ["top", "right", "left"]: axB.spines[s].set_visible(False)
axB.set_title("...at accuracy parity", fontsize=9, color=INK, loc="left")

fig.text(0.5, 0.035,
         "Honest reading: an efficiency result. Any accuracy benefit of memory is small and "
         "scales with task difficulty — not claimed as significant. Cost is vs. other LLM agents, "
         "not classical optimisation.",
         ha="center", fontsize=7.0, color="#8a5a1a",
...
print(f"OK graphical_abstract  | TaskA: {base_tok:.0f}->{rar_tok:.0f} tok ({red:.0f}% cut), "
      f"acc {base_acc:.1f}->{rar_acc:.1f}%")

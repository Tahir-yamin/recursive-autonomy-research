...
% --- Abstract -----------------------------------------------------------------
\begin{abstract}
We establish that local Recursive Language Model context compression physically reduces prompt context density by 72.5\% and net token cost by 70.0\% without compromising search generalization in autonomous agents. Across long-horizon machine learning optimization loops, standard agents suffer from context rot---an inference-time reasoning degradation where accumulated logs displace task-critical findings. We introduce Recursive Autonomous Research (RAR), an agentic architecture that eliminates context rot through recursive context compression, an iterative propose-train-evaluate ratchet, and persistent GraphRAG memory consolidation. We evaluate our architecture by training high-dimensional PyTorch Residual MLP (\texttt{DynamicMLP}) models on a dynamically generated Gaussian quantile classification manifold across $N=10$ independent random seeds over 60-cycle campaigns. RAR preserves search coherence and achieves test accuracy statistically equivalent to the stateless baseline: a paired two one-sided test (TOST) establishes equivalence within a $\pm 2$ percentage-point margin ($p = 0.003$; 95\% CI on the mean difference $[-0.65, +1.40]$ points), while a one-sided superiority test is non-significant ($40.50 \pm 1.68\%$ vs $40.12 \pm 1.20\%$; Wilcoxon $p = 0.25$). RAR attains this equivalent accuracy while cutting net token cost by 70.0\% and reducing late-cycle exploration redundancy roughly four-fold relative to a compute-matched Vector RAG baseline. Local Write-Ahead Logging (WAL) state protection secures 100\% task recoverability under induced API failures. These findings demonstrate that structured context pruning preserves search integrity at a fraction of the token budget---an efficiency result, established as accuracy-equivalence rather than accuracy-superiority.
\end{abstract}

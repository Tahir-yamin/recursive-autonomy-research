...
\begin{enumerate}[label=\textbf{C\arabic*.},leftmargin=2.2em,itemsep=2pt]
  ...
  \item A three-condition validation study across three classification benchmarks of graded difficulty ($N{=}10$ seeds, 60 cycles each) tuning PyTorch Residual MLP (\texttt{DynamicMLP}) models, validated against an offline, locked Test Vault (\cref{sec:experiments}). We establish that context compression reduces net token cost by $\sim 68\%$ while holding test accuracy at parity with the stateless baseline.
...
\midrule
\multicolumn{7}{L{15.5cm}}{\emph{Efficiency} (synthetic campaign, per condition): net tokens $350{,}249$ / $170{,}502$ / $\mathbf{105{,}055}$ ($\mathbf{-70\%}$ vs baseline); prompt density $\delta$ $1.41$ / $0.66$ / $\mathbf{0.39}$ ($\mathbf{-72.5\%}$); late-cycle redundancy $0.4$ / $33.2$ / $7.8$; SRE WAL recoverability $0\%$ / $100\%$ / $\mathbf{100\%}$.} \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{figure}[htbp]
  ...
  \caption{\textbf{What RAR cuts at equal accuracy} (synthetic campaign). Relative to the Stateless Baseline, RAR reduces net token cost by 70.0\%, context density by 72.5\%, and per-seed wall-clock latency by 29.2\%, while test accuracy stays at parity with the baseline across all three benchmarks (\cref{tab:empirical_results}).}
  \label{fig:reductions}
\end{figure}

\paragraph{Efficiency is the robust effect.} On the synthetic campaign the stateless baseline's logs grew until prompt density reached $\delta = 1.41$, far beyond the compression regime, whereas RAR held $\delta = 0.39$ and the compute-matched Vector RAG $0.66$. RAR cut net token cost to $105{,}055$ characters, a $70\%$ reduction over the baseline's $350{,}249$, and held late-cycle redundancy to $7.8$ against Vector RAG's $33.2$. This efficiency gain replicates across the harder benchmarks and is the result we state without qualification: RAR sustains the agent's search at roughly one-third of the token budget.
...
\paragraph{Vector RAG stagnates.} The compute-matched retrieval baseline tracks the stateless agent on accuracy on every benchmark while incurring by far the highest exploration redundancy ($33.2$ on the synthetic campaign). By repeatedly surfacing and re-proposing nearby high-scoring neighbours without community-level synthesis, it spends its token budget re-evaluating known regions. RAR's compression reaches the same accuracy at a fraction of both the token cost and the redundancy.
...
\begin{figure*}[!t]
  ...
  \caption{\textbf{Synthetic-manifold campaign ($N{=}10$ seeds, 60 cycles).} Left: the test-accuracy distribution across all ten seeds per condition (box = interquartile range, points = individual seeds); RAR edges the baseline (8/10 seeds) but the Wilcoxon test is not significant ($p = 0.116$). Right: mean net token cost per condition---RAR uses 70.0\% fewer tokens than the Stateless Baseline and 38.4\% fewer than Vector RAG. Together the panels show RAR's small accuracy edge at a fraction of the token budget.}
  \label{fig:crs}
\end{figure*}
...
\begin{enumerate}[label=\textbf{L\arabic*.},leftmargin=2.2em,itemsep=3pt]
  ...
  \item \textbf{Mixed proposing model on CIFAR:} The CIFAR $N{=}10$ aggregates six seeds proposed by \texttt{gemma2:9b} (local) and four by an OpenRouter model, a consequence of compute-availability constraints. The two halves use the same task, splits, and offline metric, and each seed is paired across conditions, so the within-task comparison is valid; but a single uniform orchestrator would be cleaner. \textit{Action: re-run all ten CIFAR seeds under one orchestrator.}
  \item \textbf{Statistical power:} With $N{=}10$ seeds the superiority test is underpowered for sub-point effects. CIFAR reaches raw $p = 0.064$, suggesting a larger seed count could clarify whether the trend is genuine. \textit{Action: increase seed count and pre-register the effect-size threshold.}
